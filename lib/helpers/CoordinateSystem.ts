import type { ClientPoint, ICoordinateSystem, Point, ScaleExtents, Size, View, ViewListener, ViewPoint } from "../types/frontendTypes";
import { NULL_BOUNDS } from "./coordinateSystemHelpers";

/**
 * Facilitates calculation and manipulation of a zoom-and-pannable view within a
 * canvas.
 */
export default class CoordinateSystem implements ICoordinateSystem {
     ...
     * @private
     */
    _view: View = { scale: 1.0, x: 0, y: 0 };

    /**
     * @private
     */
    _viewChangeListeners: Set<ViewListener> = new Set();
     ...
     */
    constructor(scaleExtents: ScaleExtents, documentSize: Size) {
     ...
     */
    scaleAtClientPoint = (deltaScale: number, clientPoint: ClientPoint) => {
     ...
     * assumed.
     */
    clientPointToViewPoint = ({ clientX, clientY }: ClientPoint, view?: View): ViewPoint => {
        if(!view) {
            view = this._view;
        }
     ...
     * assumed.
     */
    viewPointToClientPoint = ({ x, y }: Point, view: View = this._view): ClientPoint => {
        const { left, top } = this.canvasRect || NULL_BOUNDS;
     ...
     */
    attachViewChangeListener = (listener: ViewListener) => {
        this._viewChangeListeners.add(listener);
    };

    detachViewChangeListener = (listener: ViewListener) => {
        this._viewChangeListeners.delete(listener);
    };
}